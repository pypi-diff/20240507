# Comparing `tmp/swarmtube-0.1.0-py2.py3-none-any.whl.zip` & `tmp/swarmtube-0.1.31-py2.py3-none-any.whl.zip`

## zipinfo {}

```diff
@@ -1,48 +1,48 @@
-Zip file size: 46566 bytes, number of entries: 46
--rw-rw-r--  2.0 unx     1683 b- defN 24-May-02 16:20 swarmtube/__init__.py
--rw-rw-r--  2.0 unx      639 b- defN 24-May-02 16:20 swarmtube/definitions.py
+Zip file size: 47165 bytes, number of entries: 46
+-rw-rw-r--  2.0 unx     1684 b- defN 24-May-07 05:03 swarmtube/__init__.py
+-rw-rw-r--  2.0 unx      665 b- defN 24-May-04 08:01 swarmtube/definitions.py
 -rw-rw-r--  2.0 unx     6691 b- defN 24-May-02 16:20 swarmtube/swarmtube.py
 -rw-rw-r--  2.0 unx     1780 b- defN 24-May-02 16:20 swarmtube/cli/__init__.py
 -rw-rw-r--  2.0 unx     3834 b- defN 24-May-02 16:20 swarmtube/cli/config.py
 -rw-rw-r--  2.0 unx     2279 b- defN 24-May-02 16:20 swarmtube/cli/inventory.py
 -rw-rw-r--  2.0 unx     1577 b- defN 24-May-02 16:20 swarmtube/cli/main.py
 -rw-rw-r--  2.0 unx     1800 b- defN 24-May-02 16:20 swarmtube/cli/publish.py
 -rw-rw-r--  2.0 unx     2160 b- defN 24-May-02 16:20 swarmtube/cli/script.py
 -rw-rw-r--  2.0 unx     3836 b- defN 24-May-02 16:20 swarmtube/cli/setup.py
--rw-rw-r--  2.0 unx    11319 b- defN 24-May-03 04:10 swarmtube/cli/surreal.py
+-rw-rw-r--  2.0 unx    11372 b- defN 24-May-03 18:12 swarmtube/cli/surreal.py
 -rw-rw-r--  2.0 unx     1636 b- defN 24-May-02 16:20 swarmtube/cli/sync.py
 -rw-rw-r--  2.0 unx     2128 b- defN 24-May-02 16:20 swarmtube/cli/task.py
 -rw-rw-r--  2.0 unx    17629 b- defN 24-May-02 16:20 swarmtube/cli/wingdbstub.py
 -rw-rw-r--  2.0 unx      688 b- defN 24-May-02 16:20 swarmtube/cli/workspace.py
 -rw-rw-r--  2.0 unx       23 b- defN 24-May-02 16:20 swarmtube/helpers/__init__.py
 -rw-rw-r--  2.0 unx     1222 b- defN 24-May-02 16:20 swarmtube/helpers/faker.py
 -rw-rw-r--  2.0 unx     7449 b- defN 24-May-02 16:20 swarmtube/helpers/general.py
 -rw-rw-r--  2.0 unx     4264 b- defN 24-May-02 16:20 swarmtube/helpers/loaders.py
 -rw-rw-r--  2.0 unx      725 b- defN 24-May-02 16:20 swarmtube/helpers/models.py
 -rw-rw-r--  2.0 unx     3388 b- defN 24-May-02 16:20 swarmtube/helpers/surreal.py
 -rw-rw-r--  2.0 unx      134 b- defN 24-May-02 16:20 swarmtube/logic/__init__.py
 -rw-rw-r--  2.0 unx     4448 b- defN 24-May-02 16:20 swarmtube/logic/crawler.py
 -rw-rw-r--  2.0 unx      611 b- defN 24-May-02 16:20 swarmtube/logic/inventory.py
--rw-rw-r--  2.0 unx    18514 b- defN 24-May-03 05:14 swarmtube/logic/swarmtube.py
+-rw-rw-r--  2.0 unx    20338 b- defN 24-May-06 13:30 swarmtube/logic/swarmtube.py
 -rw-rw-r--  2.0 unx       24 b- defN 24-May-02 16:20 swarmtube/mappers/__init__.py
 -rw-rw-r--  2.0 unx     1025 b- defN 24-May-02 16:20 swarmtube/mappers/inventory.py
 -rw-rw-r--  2.0 unx       24 b- defN 24-May-02 16:20 swarmtube/models/__init__.py
 -rw-rw-r--  2.0 unx     3568 b- defN 24-May-02 16:20 swarmtube/models/base.py
 -rw-rw-r--  2.0 unx     1887 b- defN 24-May-02 16:20 swarmtube/models/config.py
 -rw-rw-r--  2.0 unx       34 b- defN 24-May-02 16:20 swarmtube/models/enums.py
 -rw-rw-r--  2.0 unx     1886 b- defN 24-May-02 16:20 swarmtube/models/inventory.py
 -rw-rw-r--  2.0 unx     1675 b- defN 24-May-02 16:20 swarmtube/models/script.py
 -rw-rw-r--  2.0 unx     1907 b- defN 24-May-02 16:20 swarmtube/models/swarmtube.py
 -rw-rw-r--  2.0 unx     1649 b- defN 24-May-02 16:20 swarmtube/models/task.py
 -rw-rw-r--  2.0 unx      555 b- defN 24-May-02 16:20 swarmtube/ports/__init__.py
 -rw-rw-r--  2.0 unx     1389 b- defN 24-May-02 16:20 swarmtube/ports/preferences.py
 -rw-rw-r--  2.0 unx      182 b- defN 24-May-02 16:20 swarmtube/storage/__init__.py
 -rw-rw-r--  2.0 unx     1330 b- defN 24-May-02 16:20 swarmtube/storage/surreal.py
--rw-rw-r--  2.0 unx      168 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/AUTHORS.rst
--rw-rw-r--  2.0 unx     1075 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/LICENSE
--rw-rw-r--  2.0 unx     2200 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/METADATA
--rw-rw-r--  2.0 unx      110 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/WHEEL
--rw-rw-r--  2.0 unx       49 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/entry_points.txt
--rw-rw-r--  2.0 unx       10 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/top_level.txt
--rw-rw-r--  2.0 unx     3807 b- defN 24-May-03 06:25 swarmtube-0.1.0.dist-info/RECORD
-46 files, 125011 bytes uncompressed, 40534 bytes compressed:  67.6%
+-rw-rw-r--  2.0 unx      168 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/AUTHORS.rst
+-rw-rw-r--  2.0 unx     1075 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/LICENSE
+-rw-rw-r--  2.0 unx     2201 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/METADATA
+-rw-rw-r--  2.0 unx      110 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/WHEEL
+-rw-rw-r--  2.0 unx       49 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/entry_points.txt
+-rw-rw-r--  2.0 unx       10 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/top_level.txt
+-rw-rw-r--  2.0 unx     3814 b- defN 24-May-07 05:04 swarmtube-0.1.31.dist-info/RECORD
+46 files, 126923 bytes uncompressed, 41119 bytes compressed:  67.6%
```

## zipnote {}

```diff
@@ -111,29 +111,29 @@
 
 Filename: swarmtube/storage/__init__.py
 Comment: 
 
 Filename: swarmtube/storage/surreal.py
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/AUTHORS.rst
+Filename: swarmtube-0.1.31.dist-info/AUTHORS.rst
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/LICENSE
+Filename: swarmtube-0.1.31.dist-info/LICENSE
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/METADATA
+Filename: swarmtube-0.1.31.dist-info/METADATA
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/WHEEL
+Filename: swarmtube-0.1.31.dist-info/WHEEL
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/entry_points.txt
+Filename: swarmtube-0.1.31.dist-info/entry_points.txt
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/top_level.txt
+Filename: swarmtube-0.1.31.dist-info/top_level.txt
 Comment: 
 
-Filename: swarmtube-0.1.0.dist-info/RECORD
+Filename: swarmtube-0.1.31.dist-info/RECORD
 Comment: 
 
 Zip file comment:
```

## swarmtube/__init__.py

```diff
@@ -29,8 +29,8 @@
 11. [ ] define test cases (tests/*.py) and get main
 12. [ ] implement automatic test cases
 
 """
 
 __author__ = """Asterio Gonzalez"""
 __email__ = "asterio.gonzalez@gmail.com"
-__version__ = "0.1.0"
+__version__ = "0.1.31"
```

## swarmtube/definitions.py

```diff
@@ -12,14 +12,16 @@
 
 TAG_KEY = "tags"
 
 UID_TYPE = str
 
 TAG_KEY = "tags"
 
+MONOTONIC_KEY = 'wave__'
+
 # ----------------------------------------------------------
 # Namespaces
 # ----------------------------------------------------------
 
 TEST_NS = "test"
 APP_NS = "app"
 MAPPING_NS = "mapping"
```

## swarmtube/cli/surreal.py

```diff
@@ -24,14 +24,16 @@
 # from bigplanner.models.task import PlannerTaskResponse as Response
 
 from syncmodels.helpers.surreal import SurrealServer
 from syncmodels.helpers.faker import fake
 
 from surrealist import Surreal
 
+
+from ..definitions import MONOTONIC_KEY
 # ---------------------------------------------------------
 # Dynamic Loading Interface / EP Exposure
 # ---------------------------------------------------------
 TAG = "SURREAL"
 DESCRIPTION = "SurrealDB CLI API"
 API_ORDER = 10
 
@@ -306,15 +308,15 @@
     print(surreal.version())  # prints server version
 
     uid = "clock"
     with surreal.connect() as conn:
         while events != 0:
             t0 = time.time()
             data = {
-                "_wave": t0,
+                MONOTONIC_KEY: t0,
                 #'uid': uid,
                 "payload": events,
             }
             conn.create(uid, data)
             log.info(f"{data}")
             time.sleep(rate)
             events -= 1
@@ -365,15 +367,15 @@
 
     conn = surreal.connect()
 
     def publish(idx: int):
         uid = uids[idx]
         t0 = time.time()
         data = {
-            "_wave": t0,
+            MONOTONIC_KEY: t0,
             "payload": temps[0][idx],
         }
         conn.create(uid, data)
         log.info(f"+ [{idx}]: {data}")
 
     counter = list(inertia)
     while events > 0:
```

## swarmtube/logic/swarmtube.py

```diff
@@ -1,24 +1,35 @@
 """Main module."""
 
 # library modules
 import asyncio
 import random
 import time
+import sys
+import traceback
 import functools
 from typing import Callable, List, Dict, Any
 from pydantic import BaseModel
 
 from agptools.progress import Progress
 
+from syncmodels.storage import (
+    Storage, 
+    TUBE_SYNC,
+    TUBE_META,
+    TUBE_WAVE,
+    SurrealistStorage,
+)
+
 
 # ---------------------------------------------------------
 # local imports
 # ---------------------------------------------------------
 # from ..helpers import expandpath
+from ..definitions import MONOTONIC_KEY
 
 # ---------------------------------------------------------
 # models / mappers
 # ---------------------------------------------------------
 # from ..models.swarmtube import SwarmtubeApp
 # from .. import mappers
 # from ..models.enums import *
@@ -35,22 +46,22 @@
 """
 USE NS test DB test;
 INFO FOR NS;
 INFO FOR DB;
 --SELECT * FROM bigplanner_models_inventory_PlannerItem;
 --SELECT COUNT() FROM person GROUP ALL;
 -- SELECT * FROM person LIMIT 1;
--- SELECT * FROM clock ORDER BY _wave ASC LIMIT 1000;
--- SELECT * FROM clock WHERE _wave > 0 ORDER BY _wave ASC LIMIT 2;
+-- SELECT * FROM clock ORDER BY wave__ ASC LIMIT 1000;
+-- SELECT * FROM clock WHERE wave__ > 0 ORDER BY _wave ASC LIMIT 2;
 --SELECT * FROM random_plus_one;
--- DELETE FROM clock, random_plus_one, _waves;
---SELECT _wave, payload FROM random_plus_one ORDER BY _wave ASC;
-SELECT * FROM _waves;
---SELECT math::max(random_plus_one._wave);
--- RETURN math::max([SELECT random_plus_one["_wave"] FROM random_plus_one]);
+-- DELETE FROM clock, random_plus_one, waves__;
+--SELECT wave__, payload FROM random_plus_one ORDER BY _wave ASC;
+SELECT * FROM waves__;
+--SELECT math::max(random_plus_one.wave__);
+-- RETURN math::max([SELECT random_plus_one["wave__"] FROM random_plus_one]);
 SELECT COUNT() FROM clock GROUP ALL;
 SELECT COUNT() FROM random_plus_one GROUP ALL;
 """
 
 
 UID = str
 WAVE = float
@@ -59,27 +70,27 @@
 class Event(BaseModel):
     "TBD"
     wave: WAVE
     uid: UID
     payload: Any = None
 
 
-class Storage:
+class Hide_Storage:
     """Abstract Interface for Storage"""
 
     def __init__(self):
         pass
 
     async def start(self):
         "any action related to start storage operations"
 
     async def stop(self):
         "any action related to stop storage operations"
 
-    async def last_wave(self, uid: UID) -> WAVE:
+    async def last_waves(self, uid: UID) -> WAVE:
         "Get the last wave for a given uid"
 
     async def get(self, uid: UID, wave: WAVE, max_results=1000) -> List[Event]:
         "get data order than the given wave (i.e. > wave)"
 
     def put(self, uid: UID, edge):
         "put data into storage"
@@ -152,15 +163,15 @@
         super().__init__(uid=uid, broker=broker, storage=storage, meta=meta)
         self.sources = sources
 
 
 class App(iAgent):
     "TBD"
 
-    def __init__(self, uid='app', *args, **kw):
+    def __init__(self, uid="app", *args, **kw):
         super().__init__(uid=uid, *args, **kw)
         self.tubes = {}
         self.tasks = {}
         self.loop = None
         self.t0 = 0
 
     async def _start_live(self):
@@ -247,15 +258,15 @@
     async def _stop_live(self):
         await super()._stop_live()
 
     async def _idle(self):
         await super()._idle()
         self.counter += 1
         edge = {
-            "_wave": time.time(),  # TODO: let the storage set this value?
+            MONOTONIC_KEY: time.time(),  # TODO: let the storage set this value?
             #'uid': uid,
             "payload": self.counter,
         }
         self.storage.put(self.uid, edge)
 
 
 class Particle(Tube):
@@ -310,47 +321,48 @@
         "TBD"
         for uid in self.sources:
             await self.broker.unsubscribe(uid, self.live)
 
     async def _start_historical(self):
         "TBD"
         self.state = ST_HISTORICAL
-        _wave = await self.storage.last_wave(self.uid)
-        self.wave0 = {uid: _wave for uid in self.sources}
+        # TODO: review last_waves()
+        self.wave0 = self.storage.last_waves(self.sources, self.uid)
+        #self.wave0 = {uid: _wave for uid in self.sources}
         buffer = self._historical
         self._milk = list(self.sources)
         log.info("-" * 70)
         log.info("[%s] -- Switching to HISTORICAL", self.uid)
         log.info("-" * 70)
         while self.state < ST_LIVE:
             # load some historical data
             n = 0
             while self._milk:
                 uid = self._milk.pop()
-                data = await self.storage.get(
+                data = self.storage.since(
                     uid, self.wave0[uid], max_results=self.MAX_EVENTS
                 )
                 if data:
                     buffer[uid].extend(data)
                     n += len(data)
-                    self.wave0[uid] = data[-1]["_wave"]
+                    self.wave0[uid] = data[-1][MONOTONIC_KEY]
                     break
 
             if n == 0:  # no more historical data
                 # move live data to historical and try to continue until
                 # we get a state with no more historical data and no more live data
                 self.state = ST_SWITCHING
                 # time.sleep(0.9)
                 assert id(buffer) != id(self.buffer)
                 for uid, _buff in self.buffer.items():
                     _hist = self._historical[uid]
                     while True:
                         try:
                             candidate = _buff.pop(0)
-                            if candidate["_wave"] > self.wave0[uid]:
+                            if candidate[MONOTONIC_KEY] > self.wave0[uid]:
                                 _hist.append(candidate)
                                 n += 1
                             else:
                                 # this live event has been captured by historical polling
                                 # so is already processed
                                 # print(f"*** already processed: --> {candidate}")
                                 foo = 1
@@ -405,15 +417,15 @@
 
     def pop_edge(self, buffer):
         "analyze buffer and return edge if all data is available for processing next step"
         # TODO: implement a policy delegation criteria to know when edge is ready to be processed
         waves = {}
         for uid, data in buffer.items():
             if data:
-                waves[uid] = data[0]["_wave"]
+                waves[uid] = data[0][MONOTONIC_KEY]
 
         # have we all needed data?
         if len(waves) == len(self.sources):
             _uid, _wave, edge = self._pop_policy(buffer, waves)
             self.wave0[_uid] = _wave
             return _uid, _wave, edge
         return None, None, None
@@ -424,42 +436,72 @@
         - drop the input related with the minimal wave
         - return a custom `edge` for computation
         """
         edge = {}
         _wave = min(waves.values())
         for uid, data in buffer.items():
             item = data[0]
-            if _wave == item["_wave"]:
+            if _wave == item[MONOTONIC_KEY]:
                 data.pop(0)
                 _uid = uid
             edge[uid] = item
 
-        edge["_wave"] = _wave
+        edge[MONOTONIC_KEY] = _wave
         return _uid, _wave, edge
 
     def dispatch(self, edge):
         "TBD"
         # build the data to be processed
-        ikeys = set([k for k in edge if k.startswith("_")])
+        # split metadata (".*__" fields by now) and each
+        # input stream
+        # TODO: review the criteria for keywords filtering
+        ikeys = set([k for k in edge if k.endswith("__")])
         ekeys = ikeys.symmetric_difference(edge)
         assert ekeys, "no payload in the edge?"
+        
+        # set found metadata
         data = {k: edge[k] for k in ikeys}
 
-        # do the computation
-        data["payload"] = self._compute(edge, ekeys)
-
-        # store results
-        self.storage.put(self.uid, data)
-        N = sum([len(_) for _ in self.buffer.values()])
-        self.metrics.update(buffer=N)
-
         log.info(f"[%s] -> dispatch: %s", self.uid, edge)
-        log.info(f"[%s] <- dispatch: %s", self.uid, data)
+
+        try:
+            # do the computation
+            payload = self._compute(edge, ekeys)
+            if payload is None:
+                log.info(f"[%s] <- dispatch: SKIP due NO DATA", self.uid)
+            else:
+                data.update(payload)
+        
+                # store results
+                # and shift sync / waves info
+                self.storage.put(self.uid, data, mode=Storage.MODE_TUBE)
+                N = sum([len(_) for _ in self.buffer.values()])
+                self.metrics.update(buffer=N)
+        
+                log.info(f"[%s] <- dispatch: %s", self.uid, data)
+    
+            wave = data.get(MONOTONIC_KEY) # Must exist!
+            if wave:
+                self.storage.update_sync_wave(
+                    self.sources,
+                    self.uid,
+                    wave,
+                )
+            else:
+                log.error(f"data %s has no %s key", data, MONOTONIC_KEY)
+        except Exception as why:
+            log.error(why)
+            log.error("".join(traceback.format_exception(*sys.exc_info())))
+        
+        
 
     def _compute(self, edge, ekeys):
+        """
+        Return None if we don't want to store info
+        """
         raise NotImplementedError()
 
 
 # ---------------------------------------------------------
 # Surreal Implementation
 # ---------------------------------------------------------
 from surrealist import Surreal
@@ -525,15 +567,15 @@
             if not lq.callbacks:
                 conn.kill(lq.lq_uid)
                 self._live_queries.pop(uid)
         else:
             pass
 
 
-class SurrealStorage(Storage):
+class Hide_SurrealStorage(Storage):
     "Surreal storage implementation"
 
     def __init__(self, url):
         self.url = url
         self.surreal = Surreal(
             # "http://localhost:9000",
             self.url,
@@ -564,29 +606,40 @@
         LIMIT {max_results}
         """
         # if not self.connection.is_connected():
         # foo = 1
         res = self.connection.query(query)
         return res.result
 
-    async def last_wave(self, uid: UID) -> WAVE:
+    async def last_waves(self, sources: List, uid: UID) -> Dict[str, WAVE]:
         "TBD"
-        res = self.connection.select(f"_waves:{uid}")
-        result = res.result
-        if result:
-            return result[0]["_wave"]
-        return 0
+        query = f"""
+        SELECT * FROM {TUBE_SYNC} WHERE
+        source = $source AND
+        target = $target
+        """
+        waves = {}
+        for source in sources:
+            res = self.connection.query(query, variables={'source': source, 'target': uid,})
+        
+            result = res.result
+            if result:
+                waves[source] = result[0][MONOTONIC_KEY]
+            else:
+                waves[source] = 0
+            
+        return waves
 
     def put(self, uid: UID, edge):
         "put data"
         # insert data into database
         self.connection.create(uid, edge)
 
         # update known last wave for this particular uid
-        data = {"_wave": edge["_wave"]}
+        data = {MONOTONIC_KEY: edge[MONOTONIC_KEY]}
         res = self.connection.update(f"_waves:{uid}", data)
         return res.result
 
 
 # ---------------------------------------------------------
 # Example of a Particle Implementation
 # ---------------------------------------------------------
@@ -602,7 +655,13 @@
 
 
 class TempDiff(Particle):
     def _compute(self, edge, ekeys):
         X = [edge[k]["payload"] for k in ekeys]
         y = X[0] - X[-1]
         return y
+
+
+# ---------------------------------------------------------
+class TubeSync(Particle):
+    """Do nothing special, but syncronize data"""
+
```

## Comparing `swarmtube-0.1.0.dist-info/LICENSE` & `swarmtube-0.1.31.dist-info/LICENSE`

 * *Files identical despite different names*

## Comparing `swarmtube-0.1.0.dist-info/METADATA` & `swarmtube-0.1.31.dist-info/METADATA`

 * *Files 4% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: swarmtube
-Version: 0.1.0
+Version: 0.1.31
 Summary: Swarm Workflow based on Tubes
 Home-page: https://github.com/asterio.gonzalez/swarmtube
 Author: Asterio Gonzalez
 Author-email: asterio.gonzalez@gmail.com
 License: MIT license
 Keywords: swarmtube
 Classifier: Development Status :: 2 - Pre-Alpha
```

## Comparing `swarmtube-0.1.0.dist-info/RECORD` & `swarmtube-0.1.31.dist-info/RECORD`

 * *Files 8% similar despite different names*

```diff
@@ -1,46 +1,46 @@
-swarmtube/__init__.py,sha256=6fs6udIh6WmBwyGVVtMvEbWQOF3wxNxdxSDl-_KR77U,1683
-swarmtube/definitions.py,sha256=xnsjFoSrLWSo9obeGK2SD3LSGu5zvEafklBZe1alTqg,639
+swarmtube/__init__.py,sha256=bsDxIbyXgDhE7eO5cchh2Bvfcj-6pyJdpO6PRTlr2b0,1684
+swarmtube/definitions.py,sha256=aU08kgwbLPmUwZKdMYAAWNSNl5ZBIk70pkkx0VjKzlI,665
 swarmtube/swarmtube.py,sha256=uktSYYLbcB6wLd3SH6n_7MsZULi4xXl3tljJ1sCj7Js,6691
 swarmtube/cli/__init__.py,sha256=vn0EccAVZb4LjRugK__sLjr4OHBrPV8tK-ScyYSoQpc,1780
 swarmtube/cli/config.py,sha256=3oW1q326XFGlLABSZLqT-NSu3j27rhkAyXqcypBUO7Q,3834
 swarmtube/cli/inventory.py,sha256=Vnjc4uKeOspYyX5-GOvFx04F96lbzLTtla0ba33j2zY,2279
 swarmtube/cli/main.py,sha256=D7OxrEy-fy7N85CwdWOH81We37MTk2PgBrtuU_ClR28,1577
 swarmtube/cli/publish.py,sha256=JhZWxYagMQrIRgFtlaTE9XJ8saXFkhMqnFIPFAeIxwo,1800
 swarmtube/cli/script.py,sha256=gVfqffMaxgfR4wLuD2kotvo5aByncmScWRaUnSOgYXk,2160
 swarmtube/cli/setup.py,sha256=914rlhPot_EnVc7Ly5MHIb6dTfZrTv1HdT3AO6CpscA,3836
-swarmtube/cli/surreal.py,sha256=IW3rOS7nWCLl9G4EcEBEuD-WMYKbJwG4zhAwzeDeZS4,11319
+swarmtube/cli/surreal.py,sha256=VDBWjg0JAuw7FkA88OJkYxnlybVmiD5Cq1CMfEZfhmQ,11372
 swarmtube/cli/sync.py,sha256=pRwfHBMSr_lnncCMBOzqEqKzPmbVE7H5pOMyrVpfwGo,1636
 swarmtube/cli/task.py,sha256=LZNpiaufKTCKgAp-TBClg6mkgIcTUFl6qYuNfcntcfs,2128
 swarmtube/cli/wingdbstub.py,sha256=Y2qomLJT2iN4lzvSkHQ_rDcsihhsSo_P3PC20VuMq4k,17629
 swarmtube/cli/workspace.py,sha256=r6W20Gyoyn46JFjRBKedXfys7Ut2l3tdcveSA8iyTT4,688
 swarmtube/helpers/__init__.py,sha256=qZet64gMJNAAqzUdEqCV5WDk5D2Dbw1Kxlt9Jo6x3m4,23
 swarmtube/helpers/faker.py,sha256=tPtibNh28KoHb9kcwE9WaPdPrSrN6xMbCv5HhNFEVG0,1222
 swarmtube/helpers/general.py,sha256=9JodQzN-muq9x1fcIUAblsAjM8jG8faGAeKmwgpF3xI,7449
 swarmtube/helpers/loaders.py,sha256=g_aBdqFWnOVh5CzPA_cluI9hKbav-5u8prkUXTJ-4zQ,4264
 swarmtube/helpers/models.py,sha256=p6BexQJtL5hui1OiOrddgvdyyl4i3t_ngiO1tXvjGMA,725
 swarmtube/helpers/surreal.py,sha256=x7OuU4igSD9IteRahY7-GMjIxic0stpFJNgijd8rZK4,3388
 swarmtube/logic/__init__.py,sha256=kUGgcg-toK_leB9i-HhPZ7BPClBI7yecUaVPhnJ0pzI,134
 swarmtube/logic/crawler.py,sha256=vFvaWpED_vlU_ViFQIG2A9IiaK8b_HafHiwJeq5H_og,4448
 swarmtube/logic/inventory.py,sha256=qRgbQGjEO6vv2MWTSl8h3y6VBR2IxuY4bhIw1GQ2Hzg,611
-swarmtube/logic/swarmtube.py,sha256=kTpZQZnfydd0rR5d8tawQ9eoHjcxdmqVXFjMD9gjYUc,18514
+swarmtube/logic/swarmtube.py,sha256=h0lUS5JPN7dXiNGL9cMJagb6MSvOJZiNoTIMg8vZd_Y,20338
 swarmtube/mappers/__init__.py,sha256=R6ShzAMfqiKh_FSrZK7T-giu44Ioo3ePkZpsoGPdo-Y,24
 swarmtube/mappers/inventory.py,sha256=LVlPPbrUZwwBdQP4OWk1occWl8-7ccGFGe8OnCrQ1WM,1025
 swarmtube/models/__init__.py,sha256=1H3-lETubfvf57NaX29AUdoJ1JG_r9yxHSGbljiEtaM,24
 swarmtube/models/base.py,sha256=F49yBmd1J4oSzLKU1EFFm7XdFi_rjs03B99dOcu_Eeg,3568
 swarmtube/models/config.py,sha256=cMhlQzCgo0N9qBMRQEPbBj1UxAC_OZc_PNLeNSoGV2M,1887
 swarmtube/models/enums.py,sha256=QxW9YtWOVokO3gXjELfE52paN_ZUNnXhs1HBtTpvZFE,34
 swarmtube/models/inventory.py,sha256=-18lewwVXVrRlq7CfALqKiNjVdwSoKCdRX-KtN2_uD0,1886
 swarmtube/models/script.py,sha256=pwCxOht-OXeysa8c0D9M6OyUdSDe7UaHiCVxSdOVU0M,1675
 swarmtube/models/swarmtube.py,sha256=dACX0MkqchNveuuoxGyYzKvIL6zTKKBYeSmxp_41OKc,1907
 swarmtube/models/task.py,sha256=ygyl1Fd8MI_x8S4KSFo50o3_YaE5n7F9kx4390T-ItU,1649
 swarmtube/ports/__init__.py,sha256=DUg8Upo_F0Y8LxfTqh1Q8IYSChmGSp1ZlZqwI-NXdag,555
 swarmtube/ports/preferences.py,sha256=RrSG1IrHu0_eiFVr9-LhFckPHDj7RcEVQTTDhJKO_uk,1389
 swarmtube/storage/__init__.py,sha256=CSNhFbyDpDg_fxHHw9x2GcOdx8qXZTw50_cKxKgtJ4g,182
 swarmtube/storage/surreal.py,sha256=QTEikrYghbv8nyKjc02aJRBcvBlppCNKmxjI4Qned-8,1330
-swarmtube-0.1.0.dist-info/AUTHORS.rst,sha256=3ZPoqg8Aav8DSYKd0fwcwn4_5HwSiMLart0E5Un00-U,168
-swarmtube-0.1.0.dist-info/LICENSE,sha256=JqzQ9S4hVjQBOM8SiUG9-qhokt1oKfE2qJMRNa3t410,1075
-swarmtube-0.1.0.dist-info/METADATA,sha256=DECL5-XhM1HjDWOVBnWYg-sfI459c4nBtYxT6bnsgs4,2200
-swarmtube-0.1.0.dist-info/WHEEL,sha256=DZajD4pwLWue70CAfc7YaxT1wLUciNBvN_TTcvXpltE,110
-swarmtube-0.1.0.dist-info/entry_points.txt,sha256=X-CwHgWDulWyiPxEbHmHwg9mE7FxpaCOHctheYiJgeM,49
-swarmtube-0.1.0.dist-info/top_level.txt,sha256=jJMGZgOJVEYy1TfxYBXsNV5EzL59YaT1xSDO5Fe4Pr8,10
-swarmtube-0.1.0.dist-info/RECORD,,
+swarmtube-0.1.31.dist-info/AUTHORS.rst,sha256=3ZPoqg8Aav8DSYKd0fwcwn4_5HwSiMLart0E5Un00-U,168
+swarmtube-0.1.31.dist-info/LICENSE,sha256=JqzQ9S4hVjQBOM8SiUG9-qhokt1oKfE2qJMRNa3t410,1075
+swarmtube-0.1.31.dist-info/METADATA,sha256=4JQ3Y3W5erDQl1TBen0f8Ae4aoqzClxIMKIt67xEPps,2201
+swarmtube-0.1.31.dist-info/WHEEL,sha256=DZajD4pwLWue70CAfc7YaxT1wLUciNBvN_TTcvXpltE,110
+swarmtube-0.1.31.dist-info/entry_points.txt,sha256=X-CwHgWDulWyiPxEbHmHwg9mE7FxpaCOHctheYiJgeM,49
+swarmtube-0.1.31.dist-info/top_level.txt,sha256=jJMGZgOJVEYy1TfxYBXsNV5EzL59YaT1xSDO5Fe4Pr8,10
+swarmtube-0.1.31.dist-info/RECORD,,
```

